...
def create_user(db: Session, name: str, last_name: str, username: str, password: str, email: str):
    db_user = User(name=name, last_name=last_name, username=username, password=password, email=email)
    db.add(db_user)
    db.commit()
    db.refresh(db_user)
    return db_user
